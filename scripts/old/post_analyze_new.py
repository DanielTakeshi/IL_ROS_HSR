...
def is_black(p):
    b, g, r = p
    return (b < 140) and (r < 140) and (g < 140) and (b < g + 35) and (b < r + 35)

#less selective
#was needed for 3 rollouts to prevent initial state being read at 100% (just used on first stage)
def special_blue(p):
    b, g, r = p
    return (b > 150 and (r < b - 40) and (g < b - 40)) or (r < b - 30) or (g < b - 30)

def mostly_black(contour, img):
    cimg = np.zeros(np.shape(img)[0:2])
    ...
    for i in range(len(rows)):
        if is_black(img[rows[i]][cols[i]]):
            black_count += 1
    return 0.7 < (black_count * 1.0)/len(rows)
...
def get_success(img, points, special=False):
    ...
    blue_func = is_blue
    if special:
        blue_func = special_blue

    for i, row in enumerate(out):
        for j, point in enumerate(row):
            if not blue_func(point) and not is_white(point):
                mask[i][j] = 255
            else:
                mask[i][j] = 0
    ...
    # olargest = max(contours, key = lambda cnt: cv2.contourArea(cnt))
    # cv2.drawContours(out, [olargest], -1, (0,255,0), 3)

    new_contours = []
    for c in contours:
        if mostly_black(c, out):
            new_contours.append(c)

    if len(new_contours) == 0:
        percent = 100
    else:
        largest = max(new_contours, key = lambda cnt: cv2.contourArea(cnt))
...
if __name__=='__main__':
    ...
    use_special = False
    #used just for stats:stat_8; dart_adv_stats:stat8;stats_adverserial:stat9

    statdir = "supervisor_dart"
    for rnum in range(10):
        ...
        if os.path.isfile(fname):
            ...
            results = [0, 0, 0, 0]
        pickle.dump(results, open(folder + "m2_results.p", "wb"))
